...
namespace PSM6
{
    public class BetterEulerString
    {
        ...
        public BetterEulerString(double L, double n, double dt, double lPow)
        {
           ...
           // Console.WriteLine();
            ...
            for (int i = 1; i < lPow; i++)
            {
                ...
                var f_2 = new List<double>
                {
                    0
                };
                for (int j = 1; j < n; j++)
                {
                    f_2.Add(f[j] + v[j] * (dt / 2));
                    Console.Write(f_2[j] + " | ");
                }
                f_2.Add(0);
                Console.WriteLine();

                var v_2 = new List<double>
                {
                    0
                };
                for (int j = 1; j < n; j++)
                {
                    v_2.Add(v[j] + a[j] * (dt / 2));
                    Console.Write(v_2[j] + " | ");
                }
                v_2.Add(0);
                Console.WriteLine();

                var a_2 = new List<double>
                {
                    0
                };
                for (int j = 1; j < n; j++)
                {
                    a_2.Add((f_2[j + 1] - 2 * f_2[j] + f_2[j - 1]) / Math.Pow(dx, 2));
                    Console.Write(a_2[j] + " | ");
                }
                a_2.Add(0);
                Console.WriteLine();
